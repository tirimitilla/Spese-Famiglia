...
import { Repeat, Plus, Trash2, CalendarClock, Bell, Info, LayoutList, Pencil } from 'lucide-react';
                          ...
                          <div key={item.id} className={`group relative flex flex-col p-4 bg-white rounded-2xl border shadow-sm hover:shadow-md transition-all ${isDue ? 'border-red-200 bg-red-50/20' : 'border-gray-100'}`}>
                              <div className="flex justify-between items-start">
                                  ...
                                  <div className="flex gap-1 ml-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                      <button onClick={() => setEditingItem(item)} className="p-3 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all" title="Modifica"><Pencil className="w-5 h-5" /></button>
                                      <button onClick={() => onDeleteRecurring(item.id)} className="p-3 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all" title="Elimina"><Trash2 className="w-5 h-5" /></button>
                                  </div>
                              </div>
